...
// public:
    ...
    bool update_CPT(){
        ...
        if(Node_Name.compare("\"Press\"")==0) cout << "Press: CPT[4]=" << CPT[4]<< " " << "Table[4]:" << Table[4] << endl;
        ...
        for(int j=0; j<Table.size()/nvalues; j++){
            ...
            if(norm!=0){
                for(int i=0; i<nvalues; i++){
                    ...
                    if(Node_Name.compare("\"Press\"")==0 && ind==4) cout << "FUCK...\n";
        ...
        for(int j=0; j<n_cols; j++){
            if(norms[j]==0){
                ...
                for(int c=0; c<nvalues; c++){
                    ...
                    if(abs(prev-CPT[c*n_cols + j]) >= THRESHOLD){
                        cout << "In " << Node_Name;
                        printf(" CPT[%d] from %f to %f.....(%f)\n", (c*n_cols+j), prev, CPT[c*n_cols+j], (1/((float)nvalues)));
                    }
                }
            }
        }
...
void EM(vector<int>& q_indexes){
    ...
    // utill convergence
    while((!unchanged) && (num_iterations!=300)){
